...
describe("PostForm", () => {
  ...
  it("allows multiple posts on the same day", async () => {
    const user = userEvent.setup();
    ...
    await user.click(
      screen.getByRole("button", {
        name: "Schedule Post & Add Another",
      })
    );
    ...
    expect(
      store.getState().posts.posts.filter(
        (post) => post.date === "2026-08-12"
      )
    ).toHaveLength(2);
  });
