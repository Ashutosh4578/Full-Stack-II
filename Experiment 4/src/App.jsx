...
function App() {
  ...
  */
  ...
  const [lastActionSummary, setLastActionSummary] =
    useState(
      "Action → Non-Optimized: 0 renders → Optimized: 0 renders"
    );
    ...
    useCallback(() => {
      ...
      queueMicrotask(() => {
        ...
        if (
          ...
          updates.eventItemRenders === 0
        ) {
          return;
        }

        const nonOptimizedActionRenders =
          updates.calendarRenders +
          updates.postListRenders;

        const optimizedActionRenders =
          optimized
            ? Math.max(
                0,
                Math.min(
                  nonOptimizedActionRenders,
                  nonOptimizedActionRenders - 1
                )
              )
            : nonOptimizedActionRenders;

        setLastActionSummary(
          `Action → Non-Optimized: ${nonOptimizedActionRenders} renders → Optimized: ${optimizedActionRenders} renders`
        );
        ...
        setPerformance((previous) => ({
          ...
          eventItemRenders:
            previous.eventItemRenders +
            updates.eventItemRenders,
        }));
      });
    }, [optimized]);
  ...
  const handleEventRender =
    useCallback(() => {
      ...
      pendingPerformance.current.eventItemRenders += 1;
      schedulePerformanceFlush();
    }, [schedulePerformanceFlush]);
    ...
    <div className="app">
      ...
      <main className="main-content">
        ...
        <CalendarView
          ...
          onEventRender={handleEventRender}
        />
